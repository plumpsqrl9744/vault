...
| 기능 | 내용 |
| --- | --- |
| 전/후진 이동 | 전/후진 이동 조이스틱 구현 |
| 좌/우 이동 | 좌/우 이동 조이스틱 구현 |
| 홀더 상/하 이동 | 홀더 상/하 이동 기능 구현 |
| 카메라 표시 | 제어하면서 볼 수 있는 **RTSP 카메라 뷰** 구현 |
| 네트워크 설정 | 접속, 카메라 주소 등 네트워크 관련 설정 |
| 로봇 상태 표시 | 제어하면서 확인해야 할 로봇 상태 정보 표시 |
| 비상 정지 | 비상시 바로 멈출 수 있는 비상 정지 기능 |
| 명령 로깅 보기 | 제어 명령 로깅 보기 기능 |
| 스캔/주행 모드 | 스캔/주행 **토글** 형태의 주행 모드 |
| 반자동 스캔 크루저 모드 | 직접주행/반자동 스캔 크루저 모드 (**토글**) |

* 구현 메모
	* 조이스틱 → `react-native-joystick`, 상태 표시 → Zustand ([[ERobot 시스템 아키텍처]]의 로봇 상태 토픽 수신).
	* **RTSP 뷰**는 tcp-socket과 별개 스트림 — RN에서 RTSP 직재생은 네이티브 플레이어 라이브러리 필요 (예: react-native-video 계열/VLC 계열, 선정 필요 [UNCLEAR]).
	* **비상 정지**는 UI 최상위 고정 + 최우선 패킷. 단, 최종 안전은 Teensy Watchdog이 담당 (앱은 1차 수단일 뿐).
	* 크루저 모드의 실제 주행 판단은 RPi5(ROS2) 몫 — 앱은 모드 토글 명령과 상태 표시만.
