...
> 웹(erut-sono-inspect-web) 개발 생산성·아키텍처 개선 4개 축: 빌드 → 로직 → 스타일 → 구조

### 1. 빌드 및 CI/CD 성능 최적화

- **문제 상황:** 빌드 타임이 과도하게 소요되어 생산성 저하 및 CI/CD 병목 발생.
	- 기존 빌드 진행 시 증분 빌드를 통해 캐싱을 활용해 패키지 install 시간을 감소시켜 평균적으로 20~30% 시간을 감축했지만, 여전히 패키지 install에서 대략 1분 내외가 소모됨.
- **해결 방안:**
	- **패키지 관리:** `pnpm`을 도입하여 효율적인 의존성 관리를 수행합니다.
		- 현재 erut web은 **Yarn Classic v1**(EOL) — 어차피 탈출이 필요한 상태라 전환 비용이 사실상 공짜 구간. postinstall/patch-package/.npmrc 전무로 마이그레이션 저위험 확인됨. 상세: [[p-log/erut/code-guide/web_패키지매니저_pnpm_전환_검토]] · [[p-log/knowledge/패키지매니저_pnpm_vs_npm_yarn]]
	- **캐시 최적화:** 빌드 서버의 pnpm 스토어 경로(예: `/var/lib/jenkins/.pnpm-store`)를 볼륨으로 마운트하여, 의존성 설치 캐시를 재사용함으로써 네트워크 비용과 install 시간을 단축합니다.
		- pnpm의 최대 이득 구간이 정확히 이 시나리오(**warm store + clean install → 하드링크만 걸어 2~3배**). 단, 빨라지는 건 install이지 vite build 자체가 아님에 유의.
...
- **해결 방안 (3단계 중첩 적용):**
	...
	- **[단계 2] ESLint:** `eslint-plugin-import`로 레이어 간 import 경로를 즉각적으로 제한하여 IDE 레벨에서 규칙 준수 유도.
		- 현재 erut web에 `no-restricted-imports`(상대경로 상위 import 금지)는 이미 적용됨 — 레이어 방향 규칙까지 확장하는 것이 이 단계의 목표.
